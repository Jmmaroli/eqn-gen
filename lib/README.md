...
The equation generated by the framework is built exclusively from defined template product functions. These functions are defined in the `fcns` folder and made available by `fitting_functions.py`. Each function is defined by a single file, and functions can be removed or added as needed. Functions included in the repository are defined as follows.
